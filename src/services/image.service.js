...
async function generateImage(text, style, uid, email) {
  try {
    ...
    */
    ...
    // A ver quien pingo genera la imagen
    let userWhoRequests = await userService.checkUserExists(uid)

    // A ver si es premium o si no paso el limite
    if (userWhoRequests.isPremium || userWhoRequests.countGenerations < 10) {
      ...
      let options = {
        ...
        data: {
          ...
          prompt: prompt,
          negative_prompt:
            "skin, body, limbs, arms, legs, feet, hands, torso, neck, head",
          steps: 20,
          cfg_scale: 7,
          seed: -1,
          upscale: false,
      ...
      // Función para consultar el estado del job y esperar hasta que no este listo
      ...
      // Guardo imagen en database, cuento generacion usuario
      ...
      if (status === "succeeded") {
        ...
        await newImage.save()
        // Actualizo conteo generaciones cliente
        const updatedCount = (userWhoRequests.countGenerations || 0) + 1
        userWhoRequests.countGenerations = updatedCount
        await userWhoRequests.save()

        return { status, imageUrl, updatedCount }
      }
    } else {
